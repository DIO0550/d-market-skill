...
description: "Agent Skillの設計・作成・改善・評価を支援するスキル。SKILL.mdの構成、YAMLフロントマター、段階的開示パターン、ワークフロー設計、評価・イテレーションのベストプラクティスを網羅。「スキルを作りたい」「SKILL.mdの書き方」「スキル設計」「カスタムSkill作成」「スキルのベストプラクティス」「スキルの改善」「スキルのテスト」「スキル構造」「スキルの評価」「description最適化」などのキーワードでトリガー。新規スキル作成、既存スキルの改善、スキル設計のレビュー、いずれの場面でも積極的に使用すること。"
---
...
ユーザーがこのプロセスのどこにいるかを判断し、適切な地点から支援する。既にドラフトがあれば評価・改善から。テストの粒度はユーザーと相談して決める。
...
| **L3: リソース** | 必要時 | 実質無制限 | references/, templates/, scripts/ |
...
スキルドラフト作成後、2-3個のリアルなテストプロンプトを作成。ユーザーに確認してから実行。
...
2. **分析**: `agents/analyzer.md` を参照してベンチマークデータのパターンを分析

**Step 5: フィードバック**

結果をユーザーに直接提示してレビューしてもらう。具体的な不満があるテストケースに改善を集中する。
...
各クエリに対してdescriptionがトリガーするかを確認し、トリガー精度が低い場合はdescriptionを調整する。before/after とスコアをユーザーに報告。
...
| [references/schemas.md](references/schemas.md) | JSONスキーマ定義 | テストケース・採点結果の構造化時 |
