...
不明な場合はリポジトリルートを対象に全primitiveを監査する。**ユーザーと往復できない実行環境**（バッチ／CI／サブエージェント内など）では確認を待たず、この既定（リポジトリルート＋全primitive、目的は一度きりの診断）で進め、採用した前提をレポート冒頭に明記する。
...
**作業ディレクトリと成果物の配置**（明示的に従うこと）：対象の兄弟に `steering-audit-workspace/` を作成し、その**ルート直下**に最終成果物 `audit-report.md` と `audit.json` を、中間成果物は `inventory.json`・`findings/<primitive>.json`・`fit.json` として同じルート直下に置く。ユーザーが出力先を指定した場合はそちらを優先する。対象ディレクトリ自体には一切書き込まない（監査は読み取り専用）。

**サブエージェント委譲の判断**：`Agent`（サブエージェント起動）ツールが使えるなら下記フェーズどおりに委譲する。使えない環境では各フェーズの「[agents/*.md] を読み、その基準で**自分が**実行する」フォールバックに切り替える（下記「環境固有の指示」参照）。どちらを採ったかをレポートに記す。
...
出力（いずれも `steering-audit-workspace/` の**ルート直下**に置く。ユーザー指定の出力先があればそちらを優先）：

- **`audit-report.md`** — 人間可読。サマリ（primitive別スコア・健全性）、重大度順の所見、適合ミスマッチ、優先度付き改善ロードマップ。非対話環境では冒頭に採用した監査前提（対象・スコープ・委譲モード）を明記する。
